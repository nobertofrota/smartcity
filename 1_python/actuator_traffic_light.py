...
DEVICE_ID = "semaforo-1"
DEVICE_TYPE = messages_pb2.TRAFFIC_LIGHT
CONTROL_PORT = 9202
...
def state_text():
    active = "ligado" if state["active"] else "desligado"
    return f"{active}; cor={state['color']}; modo={state['mode']}"
...
def discovery_listener():
    ...
    while True:
        try:
            ...
            print(f"[TrafficLight] Discovery response sent to {gateway_ip}:{response_port} state={state_text()}")
        except Exception as exc:
            print(f"[TrafficLight] discovery error: {exc}")
...
def control_server():
    ...
    server.listen()
    print(f"[TrafficLight] control TCP on 0.0.0.0:{CONTROL_PORT}")
...
def handle_control_conn(conn):
    with conn:
        try:
            ...
            print(
                f"[TrafficLight] TCP command received command={cmd.command_type} "
            ...
            with state_lock:
                if cmd.command_type == messages_pb2.ACTIVATE:
                    ...
                    state["active"] = False
                    msg = "Semaforo desligado"
                elif cmd.command_type == messages_pb2.TRAFFIC_LIGHT_SET_COLOR:
                    color = cmd.text_value.lower()
                    if color not in {"verde", "amarelo", "vermelho"}:
                        raise ValueError("Cor deve ser verde, amarelo ou vermelho")
                    state["color"] = color
                    state["mode"] = "manual"
                    msg = f"Cor alterada para {color}"
                elif cmd.command_type == messages_pb2.TRAFFIC_LIGHT_SET_MODE:
                    mode = cmd.text_value.lower()
                    if mode not in {"automatico", "manual"}:
                        raise ValueError("Modo deve ser automatico ou manual")
                    state["mode"] = mode
                    msg = f"Modo alterado para {mode}"
                else:
                    raise ValueError("Comando invalido para semaforo")
