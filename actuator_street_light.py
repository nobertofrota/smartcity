...
DEVICE_ID = "poste-1"
DEVICE_TYPE = messages_pb2.STREET_LIGHT
CONTROL_PORT = 9203
...
def state_text():
    ...
    return f"{active}; brilho={state['brightness']:.0f}%"
...
def discovery_listener():
    ...
    while True:
        try:
            ...
            print(f"[StreetLight] Discovery response sent to {gateway_ip}:{response_port} state={state_text()}")
        except Exception as exc:
            print(f"[StreetLight] discovery error: {exc}")
...
def control_server():
    ...
    server.listen()
    print(f"[StreetLight] control TCP on 0.0.0.0:{CONTROL_PORT}")
...
def handle_control_conn(conn):
    with conn:
        try:
            ...
            print(
                f"[StreetLight] TCP command received command={cmd.command_type} "
            ...
            with state_lock:
                if cmd.command_type == messages_pb2.ACTIVATE:
                    ...
                    state["active"] = False
                    msg = "Poste desligado"
                elif cmd.command_type == messages_pb2.STREET_LIGHT_SET_BRIGHTNESS:
                    if cmd.value < 0 or cmd.value > 100:
                        raise ValueError("Brilho deve estar entre 0 e 100")
                    state["brightness"] = cmd.value
                    msg = f"Brilho alterado para {cmd.value:.0f}%"
                else:
                    raise ValueError("Comando invalido para poste")
