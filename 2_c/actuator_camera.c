...
static CameraState g_state;

static void build_state_text(char *text, size_t size) {
    snprintf(text, size, "%s; %s; direcao=%s",
             g_state.active ? "ligada" : "desligada",
             g_state.recording ? "gravando" : "sem gravacao",
             g_state.direction);
}
...
static DWORD WINAPI discovery_thread(LPVOID param) {
    ...
    for (;;) {
        ...
        resp.sensor_id = smart_dup_cstring("camera-1");
        resp.sensor_type = CAMERA;
        resp.sensor_ip = local_ip ? local_ip : smart_dup_cstring("127.0.0.1");
        resp.control_tcp_port = 9201;
...
static ControlResponse handle_command(const ControlCommand *cmd) {
    ...
    if (cmd->command_type == ACTIVATE) {
        g_state.active = true;
        resp.status = OK;
        resp.message = smart_dup_cstring("Camera ligada");
    } else if (cmd->command_type == DEACTIVATE) {
        g_state.active = false;
        g_state.recording = false;
        resp.status = OK;
        resp.message = smart_dup_cstring("Camera desligada");
    } else if (cmd->command_type == CAMERA_START_RECORDING) {
        if (!g_state.active) {
            resp.status = ERROR;
            resp.message = smart_dup_cstring("Erro no comando: Camera desligada");
        } else {
            g_state.recording = true;
            resp.status = OK;
            resp.message = smart_dup_cstring("Gravacao iniciada");
        }
    } else if (cmd->command_type == CAMERA_STOP_RECORDING) {
        g_state.recording = false;
        resp.status = OK;
        resp.message = smart_dup_cstring("Gravacao parada");
    } else if (cmd->command_type == CAMERA_SET_DIRECTION) {
        if (!cmd->text_value || cmd->text_value[0] == '\0') {
            resp.status = ERROR;
            resp.message = smart_dup_cstring("Erro no comando: Direcao obrigatoria");
        } else {
            strncpy(g_state.direction, cmd->text_value, sizeof(g_state.direction) - 1);
            g_state.direction[sizeof(g_state.direction) - 1] = '\0';
            resp.status = OK;
            {
                char buf[128];
                snprintf(buf, sizeof(buf), "Direcao alterada para %s", g_state.direction);
                resp.message = smart_dup_cstring(buf);
            }
        }
    } else {
        resp.status = ERROR;
        resp.message = smart_dup_cstring("Erro no comando: Comando invalido para camera");
    }
    resp.sensor_id = smart_dup_cstring("camera-1");
    resp.is_active = g_state.active;
    {
...
static DWORD WINAPI control_thread(LPVOID param) {
    ...
    printf("[Camera] control TCP on 0.0.0.0:9201\n");
    ...
    for (;;) {
...

        {
            ...
            free(raw);
            printf("[Camera] TCP command received command=%d value=%.2f text=%s\n",
...
int main(void) {
    ...
    strcpy(g_state.direction, "Norte");
    g_state.active = true;
