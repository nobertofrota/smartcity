...
SENSOR_ID = "noise-1"
SENSOR_TYPE = messages_pb2.NOISE_SENSOR
...
def discovery_listener():
    # Igual aos outros sensores: entra no grupo multicast e responde ao gateway.
    ...
    while True:
        try:
            ...
            resp = messages_pb2.DiscoveryResponse(
                ...
                state_text="Ruido via UDP",
            )
            sock.sendto(resp.SerializeToString(), (gateway_ip, discovery_response_port))
            print(f"[Noise] Discovery response sent to {gateway_ip}:{discovery_response_port}")
